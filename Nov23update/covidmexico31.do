...
import delimited "201123COVID19MEXICO.csv", encoding(ISO-8859-1)
...
histogram daystodeath if daystodeath<30 & covid19==1, discrete frequency ytitle(Frecuencia (muertes)) xtitle(Días desde ingreso (al 15 de noviembre)) by(, title(Dias transcurridos hasta defunción por COVID19)) scheme(s2mono) by(imss sex)
graph save Graph "diastranscurridosimss31.gph", replace
graph export "diastranscurridosimss31.png", as(png) replace 
histogram daystodeath if daystodeath<30, discrete frequency ytitle(Frecuencia (muertes)) xtitle(Días desde ingreso (al 15 de noviembre)) by(, title(Dias transcurridos hasta defunción por IRAG)) scheme(s2mono) by(hospital sex)
graph save Graph "diastranscurridoshospital31.gph", replace
graph export "diastranscurridoshospital31.png", as(png) replace 
histogram daystodeath if daystodeath<30, discrete frequency ytitle(Frecuencia (muertes)) xtitle(Días desde ingreso (al 15 de noviembre)) by(, title(Dias transcurridos hasta defunción por IRAG)) scheme(s2mono) by(covid19 sex)
graph save Graph "diastranscurridosirag31.gph", replace
graph export "diastranscurridosirag31.png", as(png) replace 
...
graph export "hazardsector31.png", as(png) replace 
sts graph if _t<=30, hazard ci by(intubate)
graph export "hazardintubate31.png", as(png) replace 
sts graph if _t<=30, hazard ci by(hospital)
graph export "hazardhospital31.png", as(png) replace 
sts graph if _t<=30, hazard ci by(icu)
graph export "hazardicu31.png", as(png) replace 
sts graph if _t<=30, hazard ci by(male)
graph export "hazardmale31.png", as(png) replace 
sts graph if _t<=30, hazard ci by(indigenous)
graph export "hazardindig31.png", as(png) replace 
sts graph if _t<=30, hazard ci by(covid19)
graph export "hazardcovid31.png", as(png) replace 
...
save "covidmexico31.dta", replace 
...
outreg2 using covidusmer31, replace tex;
logit death i.sexo##i.covid19 c.edad##i.covid19  
	i.state  
	if usmer==1 , or vce(cluster st_mun_id);
outreg2 using covidusmer31, append tex;
logit death i.sexo##i.covid19 c.edad##i.covid19  
	diabetes epoc asma inmusupr hipertension otra_con cardiovascular obesidad renal_cronica tabaquismo  
	i.state  
	if usmer==1 , or vce(cluster st_mun_id);
...
* table only for epidemic since Aug 1
set more off
#delimit ;
logit death i.sexo##i.covid19 c.edad##i.covid19   
	if usmer==1 & mes>=8, or vce(cluster st_mun_id);
outreg2 using covid90days31, replace tex;
logit death i.sexo##i.covid19 c.edad##i.covid19  
	i.state  
	if usmer==1 & mes>=8, or vce(cluster st_mun_id);
outreg2 using covid90days30, append tex;
logit death i.sexo##i.covid19 c.edad##i.covid19  
	diabetes epoc asma inmusupr hipertension otra_con cardiovascular obesidad renal_cronica tabaquismo  
	i.state  
	if usmer==1 & mes>=8, or vce(cluster st_mun_id);
outreg2 using covid90days31, append tex;
logit death i.sexo##i.covid19 c.edad##i.covid19 
	pregnant indigenous travel contagion 
	diabetes epoc asma inmusupr hipertension otra_con cardiovascular obesidad renal_cronica tabaquismo  
	i.state  
...
logit death i.sexo##i.covid19 c.edad##i.covid19 
	...
	i.state  
	if usmer==1 & mes>=8, or vce(cluster st_mun_id);
outreg2 using covi90days31, append tex;
logit death i.sexo##i.covid19 c.edad##i.covid19 
	pregnant indigenous contagion 
	diabetes epoc asma inmusupr hipertension otra_con cardiovascular obesidad renal_cronica tabaquismo 
	hospital ingresodate onset pneumonia icu intubate 
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente Grave en USMER desde Agosto 1") 
	subti("Hombre con pneumonía intubado en UCI de hospital")) 
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "90daysgrave31.gph", replace ;
graph export "90daysgrave31.png", as(png) replace ;



margins sector3, over(covid19) at(edad = (20 30 40 50 60 70 80 90) sexo=1 intubate=0 icu=0 hospital=0 pneumonia=0) ;
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente No Grave en USMER desde Agosto 1") 
	subti("Mujer Ambulatoria Sin Pneumonía")) 
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "90daysnograve31.gph", replace ;
graph export "90daysnograve31.png", as(png) replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción por Edad y Género") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "Demography31.gph", replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente Grave") 
	subti("Hombre con pneumonía intubado en UCI de hospital")) 
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "SectorGrave31.gph", replace ;
margins sector3, over(covid19) at(edad = (20 30 40 50 60 70 80 90) sexo=1 intubate=0 icu=0 hospital=0 pneumonia=0) ;
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente No Grave") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "SectorNoGrave31.gph", replace ;
...
marginsplot , bydimension(covid19) allx  xlabel(1(1)32, angle(45) labs(tiny))  byopt(ti("Defunción de Paciente No Grave") 
	...
	scheme(s1mono);
graph save Graph "EntidadNoGrave31.gph", replace ;
graph export "EntidadNoGrave31.png", as(png) replace ;
#delimit ;
...
marginsplot , bydimension(covid19) allx  xlabel(1(1)32, angle(45) labs(tiny))  byopt(ti("Defunción de Paciente Grave") 
	...
	plotopts( connect(i) pstyle(p1) )
	scheme(s1mono) ;
graph save Graph "EntidadGrave31.gph", replace ;
graph export "EntidadGrave31.png", as(png) replace ;



graph use "SectorGrave31.gph", scheme(s1color) ;
graph export "SectorGrave31.png", as(png) replace ;

graph use "SectorNoGrave31.gph", scheme(s1color) ;
graph export "SectorNoGrave31.png", as(png) replace ;

graph use "Demography31.gph", scheme(s1mono) ;
graph export "Demography31.png", as(png) replace ;
...
outreg2 using comorbidity31, replace tex;	
...
graph save Graph "ComorbOtro31.gph", replace ;
...
outreg2 using comorbidity31, append tex;	
...
graph save Graph "ComorbIMSS31.gph", replace ;
...
graph save Graph "ComorbISSSTE31.gph", replace ;
...
outreg2 using comorbidity30, append tex;	
...
graph save Graph "ComorbPrivado31.gph", replace ;
...
graph save Graph "ComorbSSA31.gph", replace ;
...
graph save Graph "ComorbUSMER31.gph", replace ;

* export as png in monochrome scheme
#delimit ;
graph use "ComorbOtro31.gph", scheme(s1mono) ;
graph export "ComorbOtro31.png", as(png) replace ;
graph use "ComorbIMSS31.gph", scheme(s1mono) ;
graph export "ComorbIMSS31.png", as(png) replace ;
graph use "ComorbISSSTE31.gph", scheme(s1mono) ;
graph export "ComorbISSSTE31.png", as(png) replace ;
graph use "ComorbPrivado31.gph", scheme(s1mono) ;
graph export "ComorbPrivado31.png", as(png) replace ;
graph use "ComorbSSA31.gph", scheme(s1mono) ;
graph export "ComorbSSA31.png", as(png) replace ;
graph use "ComorbUSMER31.gph", scheme(s1mono) ;
graph export "ComorbUSMER31.png", as(png) replace ;

#delimit ;
graph combine "ComorbIMSS31.gph" 
	"ComorbISSSTE31.gph" 
	"ComorbSSA31.gph"
	"ComorbUSMER31.gph" ,  scheme(s1color)
	...
	;
graph save Graph "ComorbEstab31.gph", replace ;
graph export "ComorbEstab31.png", as(png) replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente Grave en USMER") 
	subti("Hombre con pneumonía intubado en UCI de hospital")) 
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "usmergrave31.gph", replace ;
graph export "usmergrave31.png", as(png) replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente No Grave en USMER") 
	subti("Mujer Ambulatoria Sin Pneumonía")) 
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "usmernograve31.gph", replace ;
graph export "usmernograve31.png", as(png) replace ;
...
marginsplot ,  bydimension(covid19) byopt(ti("Defunción por Edad y Género Pacientes CDMX") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
	graph save Graph "DemographyCDMX31.gph",  replace ;
graph export "DemographyCDMX31.png", as(png) replace ;
...
marginsplot , xdimension(alcaldia) allx  xlabel(1(1)30, angle(45)) byopt(ti("Defunción de Paciente en CDMX") 
	...
	plotopts( connect(i) pstyle(p1) );
	graph save Graph "AlcaldiaCDMX31.gph",  replace ;
graph export "AlcaldiaCDMX31.png", as(png) replace ;
