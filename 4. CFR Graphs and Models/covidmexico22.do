...
* This version uses data with n=1,226,117 released August 20
...
import delimited "200820COVID19MEXICO.csv", encoding(ISO-8859-1)
...
outreg2 using covidusmer22, replace tex;
logit death i.sexo##i.covid19 c.edad##i.covid19  
	i.state  
	if usmer==1 , or vce(cluster st_mun_id);
outreg2 using covidusmer22, append tex;
logit death i.sexo##i.covid19 c.edad##i.covid19  
	diabetes epoc asma inmusupr hipertension otra_con cardiovascular obesidad renal_cronica tabaquismo  
	i.state  
	if usmer==1 , or vce(cluster st_mun_id);
...
graph save Graph "Demography22.gph", replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente Grave") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "SectorGrave22.gph", replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente No Grave") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "SectorNoGrave22.gph", replace ;
...
marginsplot , bydimension(covid19) allx  xlabel(1(1)32, angle(45) labs(tiny))  byopt(ti("Defunción de Paciente No Grave") 
	...
	scheme(s1mono);
graph save Graph "EntidadNoGrave22.gph", replace ;
graph export "EntidadNoGrave22.png", as(png) replace ;
#delimit ;
...
marginsplot , bydimension(covid19) allx  xlabel(1(1)32, angle(45) labs(tiny))  byopt(ti("Defunción de Paciente Grave") 
	...
	plotopts( connect(i) pstyle(p1) )
	scheme(s1mono) ;
graph save Graph "EntidadGrave22.gph", replace ;
graph export "EntidadGrave22.png", as(png) replace ;



graph use "SectorGrave21.gph", scheme(s1color) ;
graph export "SectorGrave21.png", as(png) replace ;

graph use "SectorNoGrave22.gph", scheme(s1color) ;
graph export "SectorNoGrave22.png", as(png) replace ;

graph use "Demography22.gph", scheme(s1mono) ;
graph export "Demography22.png", as(png) replace ;
...
graph save Graph "ComorbOtro22.gph", replace ;
...
graph save Graph "ComorbIMSS22.gph", replace ;
...
graph save Graph "ComorbISSSTE22.gph", replace ;
...
graph save Graph "ComorbPrivado22.gph", replace ;
...
graph save Graph "ComorbSSA22.gph", replace ;
...
graph save Graph "ComorbUSMER22.gph", replace ;

* export as png in monochrome scheme
#delimit ;
graph use "ComorbOtro22.gph", scheme(s1mono) ;
graph export "ComorbOtro22.png", as(png) replace ;
graph use "ComorbIMSS22.gph", scheme(s1mono) ;
graph export "ComorbIMSS22.png", as(png) replace ;
graph use "ComorbISSSTE22.gph", scheme(s1mono) ;
graph export "ComorbISSSTE22.png", as(png) replace ;
graph use "ComorbPrivado22.gph", scheme(s1mono) ;
graph export "ComorbPrivado22.png", as(png) replace ;
graph use "ComorbSSA22.gph", scheme(s1mono) ;
graph export "ComorbSSA22.png", as(png) replace ;
graph use "ComorbUSMER22.gph", scheme(s1mono) ;
graph export "ComorbUSMER22.png", as(png) replace ;

#delimit ;
graph combine "ComorbIMSS22.gph" 
	"ComorbISSSTE22.gph" 
	"ComorbSSA22.gph"
	"ComorbUSMER22.gph" ,  scheme(s1color)
	...
	;
graph save Graph "ComorbEstab22.gph", replace ;
graph export "ComorbEstab22.png", as(png) replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente Grave en USMER") 
	subti("Hombre con pneumonía intubado en UCI de hospital")) 
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "usmer22.gph", replace ;
graph export "usmer22.png", as(png) replace ;
...
marginsplot , bydimension(covid19) byopt(ti("Defunción de Paciente No Grave en USMER") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
graph save Graph "usmernograve22.gph", replace ;
graph export "usmer22nograve.png", as(png) replace ;
...
marginsplot ,  bydimension(covid19) byopt(ti("Defunción por Edad y Género Pacientes CDMX") 
	...
	xti("Edad") yti("Probabilidad Estimada") ;
	graph save Graph "DemographyCDMX22.gph",  replace ;
...
marginsplot , xdimension(alcaldia) allx  xlabel(1(1)30, angle(45)) byopt(ti("Defunción de Paciente en CDMX") 
	...
	plotopts( connect(i) pstyle(p1) );
	graph save Graph "AlcaldiaCDMX22.gph",  replace ;



graph use "DemographyCDMX22.gph", scheme(s1mono) ;
graph export "DemographyCDMX22.png", as(png) replace ;

graph use "AlcaldiaCDMX22.gph", scheme(s1mono) ;
graph export "AlcaldiaCDMX22.png", as(png) replace ;




#delimit cr
save "covidmexico22.dta", replace
